...
static esp_err_t ledoff_handler(httpd_req_t *req){
    if (gpLeaStat == 0)
    {
    digitalWrite(gpLea, LOW);
    gpLeaStat = 1;
    }
    else
    {
    digitalWrite(gpLea, HIGH);
    gpLeaStat = 0;  
    }
...
static esp_err_t leaon_handler(httpd_req_t *req){
    digitalWrite(gpLea, HIGH);
    Serial.println("LEA ON");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}

static esp_err_t leaoff_handler(httpd_req_t *req){
    digitalWrite(gpLea, LOW);
    Serial.println("LEA OFF");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}
...
void startCameraServer(){
    ...
    httpd_uri_t leaon_uri = {
        .uri       = "/leaon",
        .method    = HTTP_GET,
        .handler   = leaon_handler,
        .user_ctx  = NULL
    };
    
    httpd_uri_t leaoff_uri = {
        .uri       = "/leaoff",
        .method    = HTTP_GET,
        .handler   = leaoff_handler,
        .user_ctx  = NULL
    };
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &leaon_uri);
        httpd_register_uri_handler(camera_httpd, &leaoff_uri);
    }
